...
(defn encode-fixed
  [x baos n]
  (dotimes [i (int n)]
    (bytes.protocols/write* baos (-> (bit-shift-right x (* i 8)) (bytes.impl.core/unchecked-int) (bit-and 0xff)))))
...
(defmulti encode*
  (fn
    ([value value-type registry baos]
     (cond
       (bytes.impl.core/byte-array? value) ::byte-array
       (string? value) ::string
       (map? value) ::map
       (keyword? value) ::enum
       :else value-type))
...
(defmethod encode* ::byte-array
  ...
  (let [byte-arr-length (bytes.impl.core/alength value)]
    (varint.core/encode-uint32 byte-arr-length baos)
    (bytes.protocols/write-byte-array* baos value)))

(defmethod encode* ::string
  [value value-type registry baos & more]
  (encode* (bytes.impl.core/to-byte-array value) value-type registry baos ::byte-array))
...
(defmethod encode* ::float
  [value value-type registry baos & more]
  (encode-fixed (bytes.impl.core/float-to-raw-int-bits value) baos 4))

(defmethod encode* ::double
  [value value-type registry baos & more]
  (encode-fixed (bytes.impl.core/double-to-raw-long-bits value) baos 8))
...
(defmethod encode* ::map
  ...
  (let [value-proto (get registry value-type)]
    (doseq [[k k-value] value
      ...
      (cond
        packed?
        (let [baos-packed (bytes.impl.core/byte-array-output-stream)]
          ...
          (let [packed-byte-arr (bytes.protocols/to-byte-array* baos-packed)]
            (varint.core/encode-uint32 (bytes.impl.core/alength packed-byte-arr) baos)
        ...
        (do
          (encode-tag k-field-number k-wire-type baos)
          (let [baos-map (bytes.impl.core/byte-array-output-stream)]
...
(defmethod decode* ::byte-array
  ...
  (bytes.impl.core/to-byte-array buffer))
...
(defmethod decode* :default
  ...
  (let [value-proto (get registry value-type)]
    (loop [result {}]
      (if (== (bytes.impl.core/position buffer) (dec (bytes.impl.core/limit buffer)))
        result
        (let [{:keys [field-number wire-type]} (decode-tag buffer)
              ...
              k-buffer (cond
                         ...
                         (let [length (varint.core/decode-uint32 buffer)
                               k-buffer (bytes.impl.core/buffer-slice buffer (+ (bytes.impl.core/array-offset buffer) (bytes.impl.core/position buffer)) length)]
                           (bytes.impl.core/position buffer (+ (bytes.impl.core/position buffer) length))
                           k-buffer)
...
(comment
  ...
  (let [registry (merge default-registry
        ...
        msg
        {:type :PUT_VALUE
         :key (bytes.impl.core/byte-array 5)
         :record {:key (bytes.impl.core/byte-array 5)
                  :value (bytes.impl.core/byte-array 5)
                  :author (bytes.impl.core/byte-array 5)
                  :signature (bytes.impl.core/byte-array 5)
                  :timeReceived "1970-01-01"}
         :closerPeers [{:id (bytes.impl.core/byte-array 5)
                        :addrs [(bytes.impl.core/byte-array 5)
                                (bytes.impl.core/byte-array 5)]
                        :connection :CONNECTED}
                       {:id (bytes.impl.core/byte-array 5)
                        :addrs [(bytes.impl.core/byte-array 5)
                                (bytes.impl.core/byte-array 5)]
                        :connection :CONNECTED}]
         :providerPeers [{:id (bytes.impl.core/byte-array 5)
                          :addrs [(bytes.impl.core/byte-array 5)
                                  (bytes.impl.core/byte-array 5)]
                          :connection :CONNECTED}
                         {:id (bytes.impl.core/byte-array 5)
